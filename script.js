...
(function () {
  ...
  // Zentraler Empfänger bei Employa: JEDE Bewerbung geht – egal welche Region –
  // in Kopie an diese Adresse. Hauptempfänger bleibt PRIMARY_MAIL (du dazwischen).
  var HR_MAIL = ['christian.zapatka', 'employa.org'].join('@');
  // REGION_MAIL/REGION_FON = nur noch für die Standort-Kontaktanzeige (Telefon/Mail
  // des lokalen Büros), NICHT mehr für das Routing der Bewerbung.
  var REGION_MAIL = {
  ...
  function renderRecap() {
    ...
    if (route) {
      route.innerHTML = data.region
        ? '📬 Deine Bewerbung geht direkt an unser Recruiting-Team für <b>' + data.region + '</b>'
  ...
  function submitBewerbung() {
    ...
    var ccMail = HR_MAIL;   // IMMER an Christian, unabhängig von der Region
    ...
    if (ccMail) fd.append('_cc', ccMail);           // zentral an Christian in Kopie
    ...
    fd.append('Gewünschte Region', region || '-');
  ...
  function showSubmitError() {
    ...
    box.querySelector('a').href = 'mailto:' + PRIMARY_MAIL +
