...
(function () {
  ...
  // Der AJAX-Endpoint liefert eine lesbare Antwort, VERWIRFT aber Datei-Anhänge.
  // Nur der klassische Endpoint stellt Anhänge zu (getestet 05.07.2026) –
  // deshalb wählt submitBewerbung() den Weg je nach Upload automatisch.
  var ENDPOINT_AJAX    = 'https://formsubmit.co/ajax/' + (FORM_ALIAS || PRIMARY_MAIL);
  var ENDPOINT_CLASSIC = 'https://formsubmit.co/' + (FORM_ALIAS || PRIMARY_MAIL);
  ...
  function validateContact() {
    var anrede   = form.querySelector('#f-anrede');
    var vorname  = form.querySelector('#f-vorname');
    var nachname = form.querySelector('#f-nachname');
  ...
  // Fehler ausblenden, sobald getippt/gewählt wird
  ['#f-anrede', '#f-vorname', '#f-nachname', '#f-phone', '#f-email'].forEach(function (sel) {
    var input = form.querySelector(sel);
    var evt = input.tagName === 'SELECT' ? 'change' : 'input';
    input.addEventListener(evt, function () { input.classList.remove('invalid'); });
  });
  ...
  function collectContact() {
    ...
    data.eintritt  = formatDateDE(form.querySelector('#f-eintritt').value);
  ...
  function submitBewerbung() {
    ...
    fd.append('_subject', 'Neue Bewerbung: ' + (data.position || 'Initiativ') + ' – ' + (region || 'ohne Region') + ' – ' + data.vorname + ' ' + data.nachname);
    ...
    fd.append('Eintrittstermin ab', data.eintritt || '-');
    fd.append('Gehaltsvorstellung', data.gehalt || '-');
    fd.append('Nachricht', data.nachricht || '-');

    // Anhänge
    var cv = form.querySelector('#f-cv').files[0];
    var qu = form.querySelector('#f-quali').files[0];
    if (cv) fd.append('Lebenslauf', cv, cv.name);
    if (qu) fd.append('Qualifikationen', qu, qu.name);
    ...
    // Mit Anhängen: klassischer Endpoint – nur er stellt Dateien zu.
    // Die Antwort ist per no-cors nicht lesbar; Netzwerkfehler landen im catch.
    if (cv || qu) {
      return fetch(ENDPOINT_CLASSIC, { method: 'POST', body: fd, mode: 'no-cors' });
    }
  ...
  function buildTextSummary() {
    return [
      ...
      'Eintrittstermin ab: ' + (data.eintritt || '-'),
      'Gehaltsvorstellung: ' + (data.gehalt || '-'),
      '',
      'Nachricht:',
      (data.nachricht || '-')
    ].join('\n');
  ...
  function showSuccess() {
    document.getElementById('successName').textContent = data.vorname || 'wir haben deine Anfrage';
